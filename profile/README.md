...
graph LR
    ...
    FE -- "프레임 전송 (HTTP · ~3fps)" --> AI
    AI -- "분석 콜백" --> BE
    FE -- "세션 시작/중단" --> BE
    BE --> DB
```

프론트는 카메라 프레임을 AI 서버로 **직접 전송**하고(스트리밍이 아니라 약 330ms 간격의 HTTP 요청입니다), AI 서버는 gRPC 콜백으로 결과를 Spring에 전달합니다. 세션 시작/중단만 프론트→Spring→AI로 한 단계 거칩니다.

## 🤖 AI 서버가 만들어내는 것

| 단계 | 무엇 |
| :--- | :--- |
| **입력** | 카메라 프레임을 **약 3fps(330ms 간격)** 로 HTTP 전송 |
| **추론** | MediaPipe가 관절 좌표 33개를 추출하고, 분석에 쓰는 **13개만 남겨 저장**합니다(-60.9%) |
| **비교** | 기준 동작 시퀀스와 DTW로 정렬해 싱크로율을 계산합니다 |
| **저장** | **프레임당 1행.** rep 1회가 완성될 때 그 묶음(5~30행)을 gRPC 배치로 Spring에 넘깁니다 |

**왜 프레임마다 Spring을 부르지 않나** — rep 판정이 AI 안에서 끝나기 때문입니다. Spring은 「완성된 rep」 단위로만 받아 배치 INSERT 합니다.

**왜 3fps 상한을 두나** — rep 판정 상수가 전부 「프레임 개수」 기반이라, 클라이언트가 빨라지면 실효 시간이 짧아져 **정상 rep이 「휴식」으로 버려집니다.** 그래서 클라이언트(330ms)와 서버(300ms) 양쪽에서 상한을 겁니다.

**동시 세션 상한** — MediaPipe 검출기는 세션마다 트래킹 상태를 갖고, 인스턴스 하나가 약 100MB입니다. 그래서 검출기를 **풀에서 대여**하고, 풀 크기를 「(컨테이너 메모리 한도 - 기본 사용량) / 검출기 1개」로 계산합니다. 자리가 없으면 세션 시작을 거절합니다.

> 이 구조가 `pose_data` 를 **세션당 수백~수천 행** 규모로 만듭니다. 아래 DB 관련 작업들이 전부 여기서 출발합니다.
